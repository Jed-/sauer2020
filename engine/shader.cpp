...
void loadshaders()
{
    ...
    if(!headless && (!nullshader || !hudshader || !hudnotextureshader || !stdworldshader)) fatal("cannot find shader definitions");
...
void setshader(char *name)
{
    ...
    if(!s)
    {
        if(!headless) conoutf(CON_ERROR, "no such shader: %s", name);
    }
    else slotshader = s;
}

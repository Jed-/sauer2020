...
void printentfull(extentity &e, char *buf, int len) {
    if(efocus < 0) {
        printent(e, buf, len);
    }
    switch(e.type) {
        case ET_PARTICLES:
            if(printparticlesfull(efocus, e, buf, len)) return;
            break;
    ...
    nformatstring(buf, len, "\f2id:\f7 %d \f2pos(\f7%.2f, %.2f, %.2f\f2)\f7 %s(%d) %d %d %d %d %d", efocus, e.o.x, e.o.y, e.o.z, entities::entname(e.type), e.type, e.attr1, e.attr2, e.attr3, e.attr4, e.attr5);
}
...
ICOMMAND(entgetfull,"",  (), entfocus(efocus, string s; printentfull(e, s, sizeof(s)); result(s)));
